...
- 题目 slug：`delete-nodes-and-return-forest`
- 来源专题：链表、树与回溯
...
本题来自 `二、二叉树 / §2.4 自底向上 DFS：删点`。先把题目抽象为该分类下的标准模型：确定要维护的对象、合法状态和答案更新时机，再用来源题解正文校准细节。
...
回到来源分类 `二、二叉树 / §2.4 自底向上 DFS：删点`，选择同小节后续题目训练。若新题只是维护量变化，优先复用当前框架；若合法性条件变化，再调整枚举顺序、收缩策略或状态转移。
