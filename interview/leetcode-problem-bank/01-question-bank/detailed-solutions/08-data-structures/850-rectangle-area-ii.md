...
- LeetCode 链接：https://leetcode.cn/problems/rectangle-area-ii/
- 题目 slug：`rectangle-area-ii`
- 来源专题：常用数据结构
- 来源分类路径：二、差分 / §2.2 二维差分
- 难度分：2236
- 外部题解来源：https://leetcode.cn/problems/rectangle-area-ii/solutions/3078272/lazy-xian-duan-shu-sao-miao-xian-pythonj-4tkr/
...
- 题解标题：[Lazy 线段树 + 扫描线（Python/Java/C++/C/Go/JS/Rust）](https://leetcode.cn/problems/rectangle-area-ii/solutions/3078272/lazy-xian-duan-shu-sao-miao-xian-pythonj-4tkr/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`lazy-xian-duan-shu-sao-miao-xian-pythonj-4tkr`
- topic id：`3078272`
- 授权状态：authorized-by-user-confirmation
- 导入时间：2026-09-16 11:19:29 +0800

![lc850.png](https://pic.leetcode.cn/1739775210-DmRYLR-lc850.png)
...
考虑用 **Lazy 线段树**维护，线段树每个节点维护一段横坐标区间 $[\textit{lx}, \textit{rx}]$，记录区间内的：

- $\textit{minCoverLen}$，区间内被矩形覆盖次数最少的底边长之和。
- $\textit{minCover}$，区间内被矩形覆盖的最小次数。
- $\textit{todo}$，子树内的所有节点的 $\textit{minCover}$ 需要增加的量，注意这可以是负数。

特别地，如果 $\textit{minCover}=0$，那么 $\textit{minCoverLen}$ 就表示区间内没被矩形覆盖的底边长之和。

#### 更新操作

- 当扫描线遇到矩形的下边界时，把下边界（这是一个横坐标区间）中的 $\textit{minCover}$ 都加一。
- 当扫描线遇到矩形的上边界时，把上边界（这是一个横坐标区间）中的 $\textit{minCover}$ 都减一。
- 线段树节点的 $\textit{minCoverLen}$，等于其左右儿子 $\textit{minCover}$ 较小者的 $\textit{minCoverLen}$。如果左右儿子的 $\textit{minCover}$ 相等，则节点的 $\textit{minCoverLen}$ 等于其左右儿子的 $\textit{minCoverLen}$ 之和。
...
$$
\textit{sumLen}\cdot (y'-y)
$$

其中 $\textit{sumLen}$ 是被至少一个矩形覆盖的底边长之和，计算方式如下：

- 设最小的 $x_1$ 为 $\textit{minX}$，最大的 $x_2$ 为 $\textit{maxX}$。线段树的根节点为 $\textit{tree}[1]$，维护横坐标区间 $[\textit{minX},\textit{maxX}]$ 的信息。
- 如果 $\textit{tree}[1].\textit{minCover} > 0$，说明从 $\textit{minX}$ 到 $\textit{maxX}$ 都被至少一个矩形覆盖，那么 $\textit{sumLen} = \textit{maxX} - \textit{minX}$。
- 否则，$\textit{sumLen} = \textit{maxX} - \textit{minX} - \textit{tree}[1].\textit{minCoverLen}$，也就是减去 $[\textit{minX},\textit{maxX}]$ 中的没被矩形覆盖的底边长之和。
...
- 时间复杂度：$\mathcal{O}(n\log n)$，其中 $n$ 是 $\textit{rectangles}$ 的长度。
- 空间复杂度：$\mathcal{O}(n)$。
