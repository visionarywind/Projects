...
- 题目 slug：`range-sum-query-mutable`
- 来源专题：常用数据结构
- 来源分类路径：八、树状数组和线段树 / §8.1 树状数组
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/range-sum-query-mutable/solutions/2524481/dai-ni-fa-ming-shu-zhuang-shu-zu-fu-shu-lyfll/
...
- 题解标题：[带你发明树状数组！附数学证明（Python/Java/C++/Go/JS/Rust）](https://leetcode.cn/problems/range-sum-query-mutable/solutions/2524481/dai-ni-fa-ming-shu-zhuang-shu-zu-fu-shu-lyfll/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`dai-ni-fa-ming-shu-zhuang-shu-zu-fu-shu-lyfll`
- topic id：`2524481`
...
**证明**：按顺序拆分前缀 $[1,1],[1,2],[1,3],\ldots,[1,n]$，每次只会恰好拆出一个**新的**关键区间 $[i-\text{lowbit}(i)+1, i]$（注意 $[1, i-\text{lowbit}(i)]$ 之前拆分过了，不会产生新的关键区间），所以一共有 $n$ 个不同的关键区间。
...
更新操作证明比较复杂，推荐结合 [视频](https://www.bilibili.com/video/BV14r421W7oR?t=26m07s) 理解。
...
$$
\begin{aligned}
&5+\text{lowbit}(5)=5+1=6\\
&6+\text{lowbit}(6)=6+2=8\\
&8+\text{lowbit}(8)=8+8=16\\
\end{aligned}
$$
...
这说明随着 $i$ 的不断更新，$\text{lowbit}(i)=2^k$ 是不断增大的，那么 $k$ 也在不断增大。由于至多有 $\mathcal{O}(\log n)$ 个不同的 $k$，所以上述算法的时间复杂度为 $\mathcal{O}(\log n)$。
