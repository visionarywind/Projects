...
- LeetCode 链接：https://leetcode.cn/problems/number-of-flowers-in-full-bloom/
- 题目 slug：`number-of-flowers-in-full-bloom`
- 来源专题：常用数据结构
- 来源分类路径：二、差分 / §2.1 一维差分 / §2.1.2 进阶
- 难度分：2022
- 外部题解来源：https://leetcode.cn/problems/number-of-flowers-in-full-bloom/solutions/1445000/chai-fen-pythonjavacgo-by-endlesscheng-wz35/
...
- 题解标题：[两种方法：二分/差分（Python/Java/C++/Go/JS/Rust）](https://leetcode.cn/problems/number-of-flowers-in-full-bloom/solutions/1445000/chai-fen-pythonjavacgo-by-endlesscheng-wz35/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`chai-fen-pythonjavacgo-by-endlesscheng-wz35`
- topic id：`1445000`
...
```py [sol-Python3]
class Solution:
    def fullBlo＊＊＊lowers(self, flowers: List[List[int]], people: List[int]) -> List[int]:
...
```java [sol-Java]
class Solution {
    public int[] fullBlo＊＊＊lowers(int[][] flowers, int[] people) {
...
class Solution {
public:
    vector<int> fullBlo＊＊＊lowers(vector<vector<int>>& flowers, vector<int>& people) {
        int n = flowers.size();
...
```go [sol-Go]
func fullBlo＊＊＊lowers(flowers [][]int, people []int) []int {
	n := len(flowers)
	starts := make([]int, n)
	ends := make([]int, n)
...
```js [sol-JavaScript]
var fullBlo＊＊＊lowers = function(flowers, people) {
