...
- LeetCode 链接：https://leetcode.cn/problems/largest-local-values-in-a-matrix-ii/
- 题目 slug：`largest-local-values-in-a-matrix-ii`
- 来源专题：常用数据结构
- 来源分类路径：八、树状数组和线段树 / §8.7 ST 表（Sparse Table）
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/largest-local-values-in-a-matrix-ii/solutions/3969719/mo-ban-er-wei-st-biao-pythonjavacgo-by-e-hw5t/
...
- 题解标题：[两种方法：二维 ST 表 / 线段树套 ST 表（Python/Java/C++/Go）](https://leetcode.cn/problems/largest-local-values-in-a-matrix-ii/solutions/3969719/mo-ban-er-wei-st-biao-pythonjavacgo-by-e-hw5t/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`mo-ban-er-wei-st-biao-pythonjavacgo-by-e-hw5t`
- topic id：`3969719`
...
本题来自 `八、树状数组和线段树 / §8.7 ST 表（Sparse Table）`。先把题目抽象为该分类下的标准模型：确定要维护的对象、合法状态和答案更新时机，再用来源题解正文校准细节。
...
回到来源分类 `八、树状数组和线段树 / §8.7 ST 表（Sparse Table）`，选择同小节后续题目训练。若新题只是维护量变化，优先复用当前框架；若合法性条件变化，再调整枚举顺序、收缩策略或状态转移。
