...
- LeetCode 链接：https://leetcode.cn/problems/destroying-asteroids/
- 题目 slug：`destroying-asteroids`
- 来源专题：贪心与思维
- 来源分类路径：一、贪心策略 / §1.1 从最小/最大开始贪心
- 难度分：1335
- 外部题解来源：https://leetcode.cn/problems/destroying-asteroids/solutions/1187846/tan-xin-cong-zui-xiao-de-kai-shi-peng-zh-h7d4/
...
- 题解标题：[贪心证明，以及 O(n) 做法（Python/Java/C++/C/Go/JS/Rust）](https://leetcode.cn/problems/destroying-asteroids/solutions/1187846/tan-xin-cong-zui-xiao-de-kai-shi-peng-zh-h7d4/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`tan-xin-cong-zui-xiao-de-kai-shi-peng-zh-h7d4`
- topic id：`1187846`
...
**证法二**：证明原命题的逆否命题，即如果按照质量从小到大的顺序（记作 $B$），无法摧毁所有小行星，那么不存在可以摧毁所有小行星的顺序。设 $i$ 是最小的满足 $B_i > \textit{mass} + \sum_{j=0}^{i-1}B_j$ 的下标。把 $B$ 分成两个集合 $S = \{B_0,B_1,\ldots,B_{i-1}\}$ 和 $T = \{B_i,B_{i+1},\ldots, B_{n-1}\}$。设 $x$ 是我们尝试摧毁的**第一颗**属于 $T$ 的小行星，那么摧毁 $x$ 之前，行星的质量至多为 $M = \textit{mass} + \sum_{m\in S} m = \textit{mass}+\sum_{j=0}^{i-1}B_j$，由于 $M<B_i\le B_{i+1}\le \cdots\le B_{n-1}$，所以 $M$ 小于任何 $T$ 中的小行星质量，我们无法摧毁小行星 $x$。所以不存在可以摧毁所有小行星的顺序。

根据该命题，把 $\textit{asteroids}$ 从小到大排序，检查该顺序能否摧毁所有小行星即可。
...
- 时间复杂度：$\mathcal{O}(n\log n)$，其中 $n$ 是 $\textit{asteroids}$ 的长度。瓶颈在排序上。
...
基于这一事实，我们可以把 $\textit{asteroids}$ 中的元素按照**二进制长度**分组：
...
- 时间复杂度：$\mathcal{O}(n + \log U)$ 或 $\mathcal{O}(n)$，其中 $n$ 是 $\textit{asteroids}$ 的长度，$U=\max(\textit{asteroids})$。$\mathcal{O}(n)$ 做法可以参考【Python3 写法二】。
- 空间复杂度：$\mathcal{O}(\log U)$ 或 $\mathcal{O}(\min(n,\log U))$。
