...
- LeetCode 链接：https://leetcode.cn/problems/cat-and-mouse/
- 题目 slug：`cat-and-mouse`
- 来源专题：数学算法
- 来源分类路径：四、博弈论
- 难度分：2800
- 外部题解来源：https://leetcode.cn/problems/cat-and-mouse/solutions/3070461/ni-xiang-si-wei-tuo-bu-xu-dppythonjavacg-wp8k/
...
- 题解标题：[逆向思维 + 拓扑序 DP（Python/Java/C++/Go）](https://leetcode.cn/problems/cat-and-mouse/solutions/3070461/ni-xiang-si-wei-tuo-bu-xu-dppythonjavacg-wp8k/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`ni-xiang-si-wei-tuo-bu-xu-dppythonjavacg-wp8k`
- topic id：`3070461`
...
- $\textit{winner}[i][j][0]$ 表示鼠在节点 $i$，猫在节点 $j$，轮到**鼠**移动，最终谁获胜。
- $\textit{winner}[i][j][1]$ 表示鼠在节点 $i$，猫在节点 $j$，轮到**猫**移动，最终谁获胜。
- $\textit{winner}[i][j][k]=0$ 表示「尚未确定」或者「平局」。
- $\textit{winner}[i][j][k]=1$ 表示「鼠获胜」。
- $\textit{winner}[i][j][k]=2$ 表示「猫获胜」。

**终点**：

- 如果鼠移动到洞中（节点 $0$），那么鼠获胜。注意这里可以只考虑鼠在洞中，且轮到猫移动的状态，无需考虑继续轮到鼠移动的状态。即 $\textit{winner}[0][j][1]=1$。
- 如果猫鼠相遇，那么猫获胜。即 $\textit{winner}[i][i][0]=\textit{winner}[i][i][1]=2$。

**倒推**：

- 情况一：如果 $\textit{winner}[i][j][1]=1$，倒推上一个状态（鼠在节点 $i'$，猫在节点 $j$，轮到鼠移动），那么鼠从 $i'$ 移动到 $i$ 即可确保获胜，所以有 $\textit{winner}[i'][j][0]=1$。
- 情况二：如果 $\textit{winner}[i][j][0]=2$，倒推上一个状态（鼠在节点 $i$，猫在节点 $j'$，轮到猫移动），那么猫从 $j'$ 移动到 $j$ 即可确保获胜，所以有 $\textit{winner}[i][j'][1]=2$。
- 情况三：倒推上一个状态（鼠在节点 $i'$，猫在节点 $j$，轮到鼠移动），且无论鼠怎么移动，都是猫赢（甚至不能平局），那么标记 $\textit{winner}[i'][j][0]=2$。
- 情况四：倒推上一个状态（鼠在节点 $i$，猫在节点 $j'$，轮到猫移动），且无论猫怎么移动，都是鼠赢（甚至不能平局），那么标记 $\textit{winner}[i][j'][1]=1$。

如何判断「无论鼠怎么移动，都是猫赢」？

用**拓扑排序**处理，原理见 [210. 课程表 II](https://leetcode.cn/problems/course-schedule-ii/)。

每个状态维护相应的度数 $\textit{deg}[i][j][k]$。比如发现 $\textit{deg}[i'][j][0]=0$，但 $\textit{winner}[i'][j][0]$ 仍然是 $0$（仍然没有找到鼠赢的状态），那么标记 $\textit{winner}[i'][j][0]=2$（猫赢）。

> 注：如果最终 $\textit{deg}[i][j][k]>0$，说明这个状态是平局。

**起点（答案）**：

- 鼠在节点 $1$，猫在节点 $2$，当前轮到鼠移动，即 $\textit{winner}[1][2][0]$。

注：如果最终 $\textit{winner}[1][2][0]=0$，说明无法从终点倒推得到，也就是无法从起点移动到终点（鼠获胜或者猫获胜），即平局。

## 编程细节

设当前状态为 $\textit{winner}[i][j][k]$，那么上一个状态的 $k'=k\oplus 1$（$k$ 异或 $1$）。这样可以在 $0$ 和 $1$ 之间切换。

情况一和情况二可以合并为：如果发现 $k' = \textit{winner}[i][j][k]-1$，那么上一个状态的值就是 $\textit{winner}[i][j][k]$。

下面代码把 $\textit{graph}$ 简记为 $g$。
...
- 时间复杂度：$\mathcal{O}(n^3)$，其中 $n$ 是 $\textit{graph}$ 的长度。有 $\mathcal{O}(n^2)$ 个状态，每个状态需要遍历 $\mathcal{O}(n)$ 个相邻的状态。
- 空间复杂度：$\mathcal{O}(n^2)$。
