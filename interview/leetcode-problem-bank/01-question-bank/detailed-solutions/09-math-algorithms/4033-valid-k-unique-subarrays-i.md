...
- 来源专题：数学算法
- 来源分类路径：六、随机算法 / §6.3 异或哈希（XOR hashing）
- 难度分：2314
...
本题来自 `六、随机算法 / §6.3 异或哈希（XOR hashing）`。先把题目抽象为该分类下的标准模型：确定要维护的对象、合法状态和答案更新时机，再用来源题解正文校准细节。
...
回到来源分类 `六、随机算法 / §6.3 异或哈希（XOR hashing）`，选择同小节后续题目训练。若新题只是维护量变化，优先复用当前框架；若合法性条件变化，再调整枚举顺序、收缩策略或状态转移。
