...
- 题目 slug：`minimum-size-subarray-sum`
- 来源专题：滑动窗口与双指针
- 来源分类路径：二、不定长滑动窗口 / §2.2 越长越合法/求最短/最小
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/minimum-size-subarray-sum/solutions/1959532/biao-ti-xia-biao-zong-suan-cuo-qing-kan-k81nh/
...
- 题解标题：[【视频】两种写法：循环外更新/循环内更新，附题单（Python/Java/C++/C/Go/JS/Rust）](https://leetcode.cn/problems/minimum-size-subarray-sum/solutions/1959532/biao-ti-xia-biao-zong-suan-cuo-qing-kan-k81nh/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`biao-ti-xia-biao-zong-suan-cuo-qing-kan-k81nh`
- topic id：`1959532`
...
int minSubArrayLen(int target, int* nums, int numsSize) {
    int ans = numsSize + 1, sum = 0, left = 0;
    for (int right = 0; right < numsSize; right++) { // 枚举子数组右端点
        sum += nums[right];
        while (sum - nums[left] >= target) { // 尽量缩小子数组长度
            sum -= nums[left];
            left++; // 左端点右移
...
impl Solution {
    pub fn min_sub_array_len(target: i32, nums: Vec<i32>) -> i32 {
        ...
        for (right, &x) in nums.iter().enumerate() { // 枚举子数组右端点
            sum += x;
            while sum - nums[left] >= target { // 尽量缩小子数组长度
                sum -= nums[left];
                left += 1; // 左端点右移
...
如果 $\textit{nums}$ 中有负数，还能用滑动窗口吗？应该怎么做？

这题是 [862. 和至少为 K 的最短子数组](https://leetcode.cn/problems/shortest-subarray-with-sum-at-least-k/)，[我的题解](https://leetcode.cn/problems/shortest-subarray-with-sum-at-least-k/solutions/1925036/liang-zhang-tu-miao-dong-dan-diao-dui-li-9fvh/)。
