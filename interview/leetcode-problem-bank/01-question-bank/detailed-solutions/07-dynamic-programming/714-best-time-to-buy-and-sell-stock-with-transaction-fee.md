...
- 题目 slug：`best-time-to-buy-and-sell-stock-with-transaction-fee`
- 来源专题：动态规划
- 来源分类路径：六、状态机 DP / §6.1 买卖股票
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-with-transaction-fee/solutions/2469505/shi-pin-gu-piao-mai-mai-tong-yong-fang-f-0u38/
...
- 题解标题：[【视频】教你一步步思考动态规划，股票买卖通用方法（Python/Java/C++/Go/JS/Rust）](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-with-transaction-fee/solutions/2469505/shi-pin-gu-piao-mai-mai-tong-yong-fang-f-0u38/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`shi-pin-gu-piao-mai-mai-tong-yong-fang-f-0u38`
- topic id：`2469505`
...
本题的思路和 [122. 买卖股票的最佳时机 II](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii/solution/shi-pin-jiao-ni-yi-bu-bu-si-kao-dong-tai-o3y4/) 一样，只需要在买入（或者卖出）的时候，额外减去手续费 $\textit{fee}$ 作为开销。

为什么在买入/卖出减去都可以呢？因为最终手上一定是没有股票的（不然买了就是白白浪费钱），那么**有买入就必定有卖出**，在买入/卖出其一减去手续费就行。

注意本题 $\textit{dfs}$ 可能会算出负数，不能用 $-1$ 作为 $\textit{memo}$ 数组的初始值。可以改用其他 $\textit{dfs}$ 算不出的值。
...
impl Solution {
    pub fn max_profit(prices: Vec<i32>, fee: i32) -> i32 {
        fn dfs(idx: i32, hold: bool, prices: &[i32], memo: &mut [Vec<i32>], fee: i32) -> i32 {
            if idx < 0 {
                if hold {
