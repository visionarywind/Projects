...
- 题目 slug：`climbing-stairs`
- 来源专题：动态规划
- 来源分类路径：一、入门 DP / §1.1 爬楼梯
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/climbing-stairs/solutions/2560716/jiao-ni-yi-bu-bu-si-kao-dong-tai-gui-hua-7zm1/
...
- 题解标题：[教你一步步思考动态规划：从记忆化搜索到递推到矩阵快速幂（Python/Java/C++/Go/JS/Rust）](https://leetcode.cn/problems/climbing-stairs/solutions/2560716/jiao-ni-yi-bu-bu-si-kao-dong-tai-gui-hua-7zm1/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`jiao-ni-yi-bu-bu-si-kao-dong-tai-gui-hua-7zm1`
- topic id：`2560716`
...
请看视频 [动态规划入门：从记忆化搜索到递推](https://www.bilibili.com/video/BV1Xj411K7oF/)，其中包含把记忆化搜索 1:1 翻译成递推的技巧。
...
因为要解决的问题都是「从 $0$ 爬到 $i$」，所以定义 $\textit{dfs}(i)$ 表示从 $0$ 爬到 $i$ 有多少种不同的方法（或者说爬 $i$ 个台阶的方案数）。
...
递归入口：$\textit{dfs}(n)$，也就是答案（爬 $n$ 个台阶的方案数）。

**问**：为什么 $0$ 到 $0$ 算一种方法？

**答**：也可以这样理解，如果 $\textit{dfs}(0)=0$，那么 $\textit{dfs}(2)=\textit{dfs}(1) + \textit{dfs}(0)=1$，这显然是错误的，因为有两种方法可以爬到 $2$。所以（倒推出）$\textit{dfs}(0)$ 是 $1$。
...
具体来说，$f[i]$ 的定义和 $\textit{dfs}(i)$ 的定义是一样的，都表示从 $0$ 爬到 $i$ 有多少种不同的方法（或者说爬 $i$ 个台阶的方案数）。
...
impl Solution {
    pub fn climb_stairs(n: i32) -> i32 {
        ...
        let f0 = vec![vec![1], vec![0]];
        let fn_ = pow_mul(m, n, f0);
        fn_[0][0] as _
    }
}
```
