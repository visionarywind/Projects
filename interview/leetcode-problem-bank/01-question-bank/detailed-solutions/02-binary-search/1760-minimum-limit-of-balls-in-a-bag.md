...
- LeetCode 链接：https://leetcode.cn/problems/minimum-limit-of-balls-in-a-bag/
- 题目 slug：`minimum-limit-of-balls-in-a-bag`
- 来源专题：二分算法
- 来源分类路径：二、二分答案 / §2.4 最小化最大值
- 难度分：1940
- 外部题解来源：https://leetcode.cn/problems/minimum-limit-of-balls-in-a-bag/solutions/3071967/er-fen-da-an-pythonjavaccgojsrust-by-end-g7l7/
...
- 题解标题：[二分答案（Python/Java/C++/C/Go/JS/Rust）](https://leetcode.cn/problems/minimum-limit-of-balls-in-a-bag/solutions/3071967/er-fen-da-an-pythonjavaccgojsrust-by-end-g7l7/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`er-fen-da-an-pythonjavaccgojsrust-by-end-g7l7`
- topic id：`3071967`
- 授权状态：authorized-by-user-confirmation
- 导入时间：2026-09-16 11:19:29 +0800

假设最终每个袋子的球数都**至多**为 $m$，那么 $m$ 越小，需要的袋子越多，操作次数也越多；$m$ 越大，需要的袋子越少，操作次数也越少。有单调性，可以**二分答案**。关于二分的原理，请看视频 [二分查找 红蓝染色法【基础算法精讲 04】](https://www.bilibili.com/video/BV1AP41137w7/)。

> 或者说，看到「最小化最大值」就要先思考二分。

现在问题变成：

- 给定 $m$，要求最终每个袋子的球数都**至多**为 $m$，能否在 $\textit{maxOperations}$ 次操作内完成？

对于 $x=\textit{nums}[i]$，假设分成 $k$ 个袋子，每个袋子都至多装 $m$ 个球。$k$ 不能太小，否则没法一共装 $x$ 个球，所以 $km$ 至少要是 $x$，即
...
$$
k \ge \left\lceil\dfrac{x}{m}\right\rceil
$$

所以对于 $x$，操作次数为 

$$
\left\lceil\dfrac{x}{m}\right\rceil - 1
$$

减一是因为操作 $1$ 次分出 $2$ 个袋子，操作 $2$ 次分出 $3$ 个袋子……依此类推，操作 $k-1$ 次分出 $k$ 个袋子。

累加操作次数，判断总操作次数与 $\textit{maxOperations}$ 的大小关系。
...
- 开区间左端点初始值：$0$。每个袋子的球数不可能是 $0$，无法满足要求。
- 开区间右端点初始值：$\max(\textit{nums})$。无需操作，一定满足要求。
...
$$
\left\lceil\dfrac{x}{m}\right\rceil - 1 = \left\lfloor\dfrac{x-1}{m}\right\rfloor
$$
