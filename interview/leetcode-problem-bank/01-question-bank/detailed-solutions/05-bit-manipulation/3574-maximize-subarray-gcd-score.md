...
- LeetCode 链接：https://leetcode.cn/problems/maximize-subarray-gcd-score/
- 题目 slug：`maximize-subarray-gcd-score`
- 来源专题：位运算
- 来源分类路径：三、与或（AND/OR）的性质 / GCD LogTrick
- 难度分：Unknown
- 外部题解来源：https://leetcode.cn/problems/maximize-subarray-gcd-score/solutions/3695642/liang-chong-xie-fa-bao-li-mei-ju-logtric-zz7e/
...
- 题解标题：[两种方法：暴力枚举 / LogTrick（Python/Java/C++/Go）](https://leetcode.cn/problems/maximize-subarray-gcd-score/solutions/3695642/liang-chong-xie-fa-bao-li-mei-ju-logtric-zz7e/)
- 作者：灵茶山艾府 (`endlesscheng`)
- 题解 slug：`liang-chong-xie-fa-bao-li-mei-ju-logtric-zz7e`
- topic id：`3695642`
...
如果 $\textit{minL}\le r$，那么可以操作让 $g$ 变成 $2g$，用 $2g(i-\textit{minL}+1)$ 更新答案的最大值。
    ...
    def maxGCDScore(self, nums: List[int], k: int) -> int:
        ...
        for i, x in enumerate(nums):
            ...
            for g, l, r in intervals:
                ...
                # 看看能否乘 2
                pos = lowbit_pos[g & -g]
                min_l = max(l, pos[-k - 1]) if len(pos) > k else l  # 注意这里把 min_l 减少了 1
                if min_l < r:  # 可以乘 2
    ...
    public long maxGCDScore(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for (int j = 0; j < size; j++) {
                ...
                int minL = pos.size() > k ? Math.max(l, pos.get(pos.size() - k - 1)) : l; // 注意这里把 minL 减少了 1
                if (minL < r) { // 可以乘 2
    ...
    long long maxGCDScore(vector<int>& nums, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (auto& [g, l, r] : intervals) {
                ...
                int min_l = pos.size() > k ? max(l, pos[pos.size() - k - 1]) : l; // 注意这里把 min_l 减少了 1
                if (min_l < r) { // 可以乘 2
...
func maxGCDScore(nums []int, k int) int64 {
	...
	for i, x := range nums {
		...
		for _, p := range intervals {
			...
			minL := p.l
			if len(pos) > k {
				minL = max(minL, pos[len(pos)-k-1]) // 注意这里把 minL 减少了 1
			}
			if minL < p.r { // 可以乘 2
