...
按独立职责、接口和测试边界划分，而不是机械对应目录：`parallel_state` 与 pipeline schedule 合并为 M02，因为 schedule 的选择和 P2P 都由并行拓扑决定；optimizer 与 checkpoint 合并为 M05，因为训练状态保存接口围绕参数/优化器 shard；tools 归入 M06 仅作为外部入口，具体工具按专题可再拆。

## 深度审计

| 分析对象 | 入口落地 | 正常路径 | 分支 | 异常 | 清理 | 数据生命周期 | 执行上下文 | 行级证据 | Demo 映射 | 状态/缺口 |
|---|---|---|---|---|---|---|---|---|---|---|
| M01-M06 | 部分完成 | 部分完成 | 部分完成 | 部分完成 | 部分完成 | 部分完成 | 部分完成 | 部分完成 | D01 覆盖 M01-M05 | M06 无 D01 覆盖；详见各 README |
...
`README.md:65-87`; `gpt_builders.py:24-110`; `parallel_state.py:600-700`; `schedules.py:53-168`; `training.py:1530-1630`。

## 未解决问题

M06 内部动态 batching、跨进程通信和服务部署应在专门版本/硬件上继续核验。

## 下一步阅读建议

按 D01 顺序阅读 M02→M04→M01→M05→M03。
