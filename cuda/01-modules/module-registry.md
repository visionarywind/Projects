...
本页是 `01-modules` 的稳定导航。模块按运行时职责划分，不按目录机械拆分；源码证据均指向 `/home/mtuser/workspace/cuda`，结论区分静态确认、推断和未知。
...
| M04 | Memory/UVA/UVM | `memmgr.c`、`memblock.c`、`memobj.c`、`cuiuvm.c`、`cuivamanager.c` | 中 | 内存对象、映射、统一地址、托管内存 |
