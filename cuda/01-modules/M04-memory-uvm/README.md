...
- **职责**：把 API 内存请求变成 `CUmemdesc`、`CUmemobj`、`CUmemblock` 层次，管理 device/host/managed memory、子分配、UVA/UVM、P2P 和外部共享。
- **证据状态**：设备/host 分配、base-pointer 释放、子分配约束、UVM AL 选择、managed registration rollback、DAG/stream 状态和 P2P bookkeeping 已静态确认；fault/migration、IPC/external backend 未闭合。
- **核心文件**：`src/api/apimem.c`、`src/cui/cuimem.c`、`memobj.c`、`memblock.c`、`memmgr.c`、`cuivamanager.c`、`cuiuvm.c`。
