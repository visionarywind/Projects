...
- 文档目的：解释 01-modules/M07-flash-attention/call-chains.md 的职责、证据和维护边界。
...
本页聚焦 01-modules/M07-flash-attention/call-chains.md；具体事实以正文引用的目标源码版本为准，未执行的构建、运行和硬件行为保持未验证。
...
代表实现证据：`[kernels/flash-attn/mma/basic/flash_attn_mma_tiling_qkv.cu:94-200,201-797]`；dispatch 证据：`[kernels/flash-attn/mma/basic/flash_attn_mma_tiling_qkv.cu:799-945]`。
