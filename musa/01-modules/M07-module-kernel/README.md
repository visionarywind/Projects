# Module 与 Kernel 执行（M07）

- 源码范围：`src/driver/mu_module.cpp、context.cpp、command/dispatchCommand.cpp`
- 对应版本：`b8dce2b2f23849e8e99350c20d7eaac3c129caba`
- 证据状态：静态源码已确认；硬件行为、性能和完整异常覆盖未验证

## 一句话职责

Module 与 Kernel 执行负责 `module load -> function -> launch params -> DispatchCommand`。
